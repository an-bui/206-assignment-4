...
caption1 <- paste(strwrap("Figure 1. Lobster abundance and fishing pressure at five locations in the Santa Barbara Channel (2017). Lobster abundance (solid lines) estimated annually in late summer via SCUBA on replicate permanent SBC LTER transects at five locations: Isla Vista, Naples Reef, Arroyo Quemado, Carpenteria, Mohawk. Fishing pressure estimated by counting the number of lobster trap floats (dashed lines) in definded ares of coastline every 2 - 4 weeks during the fishing season. MPAs (green) were designated in 2012. Unprotected areas (yellow) are fished annually. Data from Reed (2017).", width = 110), collapse = "\n")
...
size_table_2017_caption = "**Table 1. Lobster carapace length in 2017.** Lobster measurements were taken at 5 SBC LTER sites in 2017. All measurements are in mm. Measurements were taken visually by estimating carapace length to the nearest ±5 mm."

kable(size_table_2017,
      col.names = size_table_2017_columns,
      caption = size_table_2017_caption) %>% 
...
size_2017_boxplot <- ggplot(abund_2017, aes(x = site_full, y = SIZE)) +
  ...
  geom_hline(yintercept = mean(abund_2017$SIZE), linetype = 2) + # add line at CL mean across sites
  #geom_jitter() +
  theme_classic() +
  theme(axis.text = element_text(color = "black"), 
...
ivee_d <- cohen.d(size_ivee_2012, size_ivee_2017, pooled = TRUE) # NEGLIGIBLE increase
aque_d <- cohen.d(size_aque_2012, size_aque_2017, pooled = TRUE) # NEGLIGIBLE increase
...
##############################
# boxplot of sizes in 2012 & 2017 in pooled MPA & non-MPA sites
##############################

# captionx <- paste(strwrap("Figure 4. blah.", width = 110), collapse = "\n") # fix alpha; bold;
# 
# abund_pooled_plot <- ggplot(abund_df, aes(x = YEAR, y = SIZE)) +
#   geom_boxplot(aes(group = YEAR, fill = factor(YEAR))) +
#   scale_fill_manual(breaks = c("2012", "2017"),values = c("white", "darkgrey"), name = "Year") +
#   facet_grid(~ protection) +
#   labs(x = "Site", y = "Carapace Length (mm)", caption = captionx) +
#   theme_classic() +
#    theme(
#     axis.title.x=element_blank(),
#     axis.text.x=element_blank(),
...
##############################
# subset pooled mpa & non-mpa for both 2012 and 2017
##############################

# size_mpa_2012 <- abund_df %>% 
#   filter(protection == "MPA",
#          YEAR == "2012") %>% 
#   pull(SIZE)
# 
# size_mpa_2017 <- abund_df %>% 
#   filter(protection == "MPA",
#          YEAR == "2017") %>% 
#   pull(SIZE)
# 
# size_nonmpa_2012 <- abund_df %>% 
#   filter(protection == "Non-MPA",
#          YEAR == "2012") %>% 
#   pull(SIZE)
# 
# size_nonmpa_2017 <- abund_df %>% 
#   filter(protection == "Non-MPA",
#          YEAR == "2017") %>% 
#   pull(SIZE)

##############################
# t-tests between 2012 & 2017 for pooled mpa and non-mpa
##############################

# mpa_t <- t.test(size_mpa_2012, size_mpa_2017) # p = 0.03
# nonmpa_t <- t.test(size_nonmpa_2012, size_nonmpa_2017) # p < 0.01 hmmmm

##############################
# Cohen's d effect size -- change in CL from 2012-2017 at pooled mpa & non-mpa
##############################

# mpa_d <- cohen.d(size_mpa_2012, size_mpa_2017) # small
# nonmpa_d <- cohen.d(size_nonmpa_2012, size_mpa_2017) # negligible
```
